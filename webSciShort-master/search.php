        ...
        <div class="container-fluid px-4 px-lg-5">
            ...
            <div class="collapse navbar-collapse" id="navbarSupportedContent">
                ...
                <!-- <form class="d-flex" action="search.php" method="get">
                    <input class="form-control me-2" type="text" placeholder="Search" aria-label="Search"name="cs_year" autocomplete="off">
...
//การค้นหาด้วยการ search ชื่อ
            ...
            //การค้นหาด้วยการ search ปี
              //  if (isset($_GET['cs_year'])) {
                 //   $cs_year = $conn->escape_string($_GET['cs_year']);
                //    $query = $conn->query("
                 //   SELECT *
                 //   FROM sci_cs 
                  //  WHERE cs_year LIKE '%{$cs_year}%'");
                    
                // search ปี พร้อม ชื่อ และไอดี
                if (isset($_GET['search'])) {
                    $search = $conn->escape_string($_GET['search']);
                    $query = $conn->query("
                    SELECT *
                    FROM sci_cs 
                    WHERE (`cs_year`LIKE '%{$search}%') OR (`cs_name`LIKE '%{$search}%') OR (`cs_id`LIKE '%{$search}%') ");
                    ?>
